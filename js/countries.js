...
let from_select = document.getElementById("from-select");
let to_select = document.getElementById("to-select");

// First option element
let option_set = `<option value="">--</option>`;

// Iterating over countries array and adding option elements with country names to the select input.
countryData.forEach(element => {
    
    let option_element =  `<option value="${element}">${element}</option>`
    option_set += option_element;
});

country_select.innerHTML = option_set;